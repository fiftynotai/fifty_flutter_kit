...
- **Restructured as template**: Moved from `packages/fifty_arch/` to `templates/mvvm_actions/`
- **Renamed**: Package name changed from `fifty_arch` to `mvvm_actions`
- **Path dependencies**: Updated to use `../../packages/` prefix
- **Clarification**: This is a project template to fork, not a package to import

### Migration

For users of the old fifty_arch:
1. Clone/fork `templates/mvvm_actions/` as your new project base
2. Import the extracted packages (fifty_utils, fifty_storage, etc.) as dependencies
3. Do NOT add mvvm_actions to your pubspec.yaml as a dependency
