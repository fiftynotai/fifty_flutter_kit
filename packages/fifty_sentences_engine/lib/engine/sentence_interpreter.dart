...
import 'sentences_engine.dart';

/// **SentenceInterpreter** - Core logic engine for sentence execution
///
/// A service responsible for interpreting and executing individual `BaseSentenceModel` instructions
...
/// - Parse and interpret the `instruction` field from a `BaseSentenceModel`
...
class SentenceInterpreter {
  ...
  final Future<void> Function(BaseSentenceModel sentence)? onWrite;

  /// Callback to show a question prompt and await a response.
  final Future<void> Function(BaseSentenceModel sentence)? onAsk;

  /// Callback to trigger navigation and wait for resume.
  final Future<void> Function(BaseSentenceModel sentence)? onNavigate;

  /// Callback to pause until the user taps or resumes manually.
  final Future<void> Function(BaseSentenceModel sentence)? onWait;

  /// Called when the sentence instruction does not match any known type.
  final Future<void> Function(BaseSentenceModel sentence)? onUnhandled;

  /// Tracks the current phase for navigation control.
  String currentPhase = '';

  /// **Interprets a single [BaseSentenceModel] and executes the defined handlers**
  ///
  ...
  Future<void> interpret(BaseSentenceModel sentence) async {
  ...
  Future<void> _handleRead(BaseSentenceModel sentence) async {
    if (onRead != null) {
