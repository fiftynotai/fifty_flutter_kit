/// Design tokens for the fifty.dev ecosystem.
///
/// This package provides the foundation design values (colors, typography,
/// spacing, motion, shadows, and breakpoints) following the Fifty Design
/// Language (FDL) v2 specification.
///
/// All tokens use FDL defaults out of the box. To customize values,
/// call [FiftyTokens.configure()] before `runApp()`.
library fifty_tokens;

export 'src/breakpoints.dart';
export 'src/colors.dart';
export 'src/config/breakpoints_config.dart';
export 'src/config/color_config.dart';
export 'src/config/fifty_tokens_config.dart';
export 'src/config/font_resolver.dart';
export 'src/config/gradients_config.dart';
export 'src/config/icon_sizes_config.dart';
export 'src/config/motion_config.dart';
export 'src/config/radii_config.dart';
export 'src/config/shadows_config.dart';
export 'src/config/spacing_config.dart';
export 'src/config/typography_config.dart';
export 'src/gradients.dart';
export 'src/icon_sizes.dart';
export 'src/motion.dart';
export 'src/preset.dart';
export 'src/radii.dart';
export 'src/shadows.dart';
export 'src/spacing.dart';
export 'src/typography.dart';
