...
///
/// Exposes narrative engine state for the view to observe.
/// All state changes flow through the NarrativeService.
library;
...
///
/// Provides a reactive interface to the NarrativeService state.
class NarrativeDemoViewModel extends ChangeNotifier {
  NarrativeDemoViewModel({
    required NarrativeService sentencesService,
  }) : _sentencesService = sentencesService {
    _sentencesService.addListener(_onServiceChanged);
  }

  final NarrativeService _sentencesService;
  ...
  NarrativeServiceState get state => _sentencesService.state;

  /// Whether the engine is idle.
  bool get isIdle => _sentencesService.state == NarrativeServiceState.idle;

  /// Whether the engine is processing.
  bool get isProcessing =>
      _sentencesService.state == NarrativeServiceState.processing;

  /// Whether the engine is paused.
  bool get isPaused => _sentencesService.state == NarrativeServiceState.paused;

  /// Whether waiting for user input (tap to continue).
  bool get isWaitingForInput =>
      _sentencesService.state == NarrativeServiceState.waitingForInput;

  /// Whether waiting for choice selection.
  bool get isWaitingForChoice =>
      _sentencesService.state == NarrativeServiceState.waitingForChoice;
  ...
  List<DemoNarrative> get queue => _sentencesService.queue;
  ...
  DemoNarrative? get currentSentence => _sentencesService.currentSentence;
