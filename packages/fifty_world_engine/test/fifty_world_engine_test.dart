...
void main() {
  ...
  group('FiftyWorldEntityExtension', () {
    test('copyWith creates new instance with overrides', () {
      final original = FiftyWorldEntity(
        id: 'original',
        type: 'room',
        asset: 'rooms/test.png',
        gridPosition: Vector2(0, 0),
    ...
    test('isMovable returns true for characters and monsters', () {
      final character = FiftyWorldEntity(
        id: 'char',
        type: 'character',
      ...
      final monster = FiftyWorldEntity(
        id: 'mon',
        type: 'monster',
      ...
      final furniture = FiftyWorldEntity(
        id: 'furn',
        type: 'furniture',
  ...
  group('FiftyWorldLoader', () {
    test('loads from JSON string', () {
      ...
      final entities = FiftyWorldLoader.loadFromJsonString(json);
    ...
    test('throws FormatException for invalid JSON', () {
      expect(
        () => FiftyWorldLoader.loadFromJsonString('not json'),
    ...
    test('throws FormatException for non-list JSON', () {
      expect(
        () => FiftyWorldLoader.loadFromJsonString('{"key": "value"}'),
        throwsA(isA<FormatException>()),
      );
    });
  });

  group('FiftyWorldUtils', () {
    test('getPositionAfterRotation returns correct offsets', () {
      final size = Vector2(100, 50);

      expect(FiftyWorldUtils.getPositionAfterRotation(0, size), isNull);
      expect(FiftyWorldUtils.getPositionAfterRotation(1, size), equals(Vector2(0, -100)));
      expect(FiftyWorldUtils.getPositionAfterRotation(2, size), equals(Vector2(100, -50)));
      expect(FiftyWorldUtils.getPositionAfterRotation(3, size), equals(Vector2(50, 0)));
      expect(FiftyWorldUtils.getPositionAfterRotation(4, size), isNull); // Same as 0
    });
    ...
    test('getAlignedPosition returns correct position', () {
      ...
      final centerPos = FiftyWorldUtils.getAlignedPosition(
        base: base,
      ...
      final topLeftPos = FiftyWorldUtils.getAlignedPosition(
        base: base,
        parentSize: parentSize,
