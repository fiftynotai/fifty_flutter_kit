...
import 'package:fifty_world_engine/src/components/text_component.dart';
...
abstract class FiftyBaseComponent extends SpriteComponent
  ...
  bool containsLocalPoint(Vector2 point) {
    final tileWidth = model.blockSize.width * FiftyWorldConfig.blockSize;
    final tileHeight = model.blockSize.height * FiftyWorldConfig.blockSize;
    return point.x >= 0 &&
        point.x < tileWidth &&
        point.y >= 0 &&
        point.y < tileHeight;
  ...
  Future<void> onLoad() async {
    ...
    if (model.quarterTurns > 0) {
      ...
      final offset =
          FiftyWorldUtils.getPositionAfterRotation(model.quarterTurns, model.size);
      if (offset != null) {
        position.add(offset);
      }
    }
...
class FiftyMovableComponent extends FiftyBaseComponent {
  ...
  void moveUp(double steps, {double speed = 200}) {
    ...
    final newPosition =
        Vector2(position.x, position.y - (steps * FiftyWorldConfig.blockSize));
  ...
  void moveRight(double steps, {double speed = 200}) {
    ...
    final newPosition =
        Vector2(position.x + (steps * FiftyWorldConfig.blockSize), position.y);
    _directMove(newPosition, speed: speed);
  }
